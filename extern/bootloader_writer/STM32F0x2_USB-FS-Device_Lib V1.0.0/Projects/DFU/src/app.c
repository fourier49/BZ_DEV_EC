...
extern void DFU_DoLeaveDFUMode(void *pdev);
...
extern int SetBootLoaderDFUMode(int StayDfu);
...
int main(void)
{
  ...
  /* USART configuration */
  USART1_Config();
  printf("[BLW]dfu boot start... %s %s vid:0x%x pid:0x%x\n\r", __DATE__, __TIME__ , USBD_VID , USBD_PID);
  ///////////////////////////////////////////////////
  ...
  if (check_enter_dfu() == 0)
  {
	...
	//BLW will stay in DFU mode for updating new BLDR,and ASK new BLDR stay in DFU when rebooting.
	SetBootLoaderDFUMode(1);
  ...
  DFU_DoLeaveDFUMode(&USB_Device_dev);
} 
