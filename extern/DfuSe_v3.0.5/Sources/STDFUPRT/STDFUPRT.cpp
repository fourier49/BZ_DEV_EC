...
extern "C" DWORD PASCAL EXPORT STDFUPRT_LaunchOperation(PDFUThreadContext pContext, PDWORD pOperationCode)
{
	...
	case OPERATION_UPLOAD: 
		{
			...
			if (pContext->ErrorCode!=STDFUPRT_NOERROR)
			{
				delete pUpload;
				dwRet=pContext->ErrorCode;
			}
			else
			{
	...
	case OPERATION_UPBLDRR:
		{
			CBIZLINKAPPThread* pRebootTh = new CBIZLINKAPPThread(pContext);
			if (pContext->ErrorCode != STDFUPRT_NOERROR)
			{
				delete pRebootTh;
				dwRet = pContext->ErrorCode;
			...
			{
				g_Operations.Add(pRebootTh);
				pRebootTh->BeginThread();
				*pOperationCode = (DWORD)pRebootTh;
			}
			break;
		}
		break;
...
extern "C" DWORD PASCAL EXPORT STDFUPRT_GetOperationStatus(DWORD OperationCode, PDFUThreadContext pContext)
{
	...
	{
		CDFUThread *pThread=(CDFUThread *)g_Operations.GetAt(i);
		if ((DWORD)pThread==OperationCode)
		{
