...
#define BIZLINKVendorID		0x06C4
#define BIZLINKPID_BLDR_OVERWRITTER	0x0202

#define USB_DFU_CLASS		0xFE
#define USB_DFU_SUBCLASS    0x1
#define USB_DFU_PROTOCOL    0x2

#define USB_BILLBOARD_CLASS		0x11
#define USB_BILLBOARD_SUBCLASS	0x0
#define USB_BILLBOARD_PROTOCOL	0x0

#define USB_GOOGLE_CONSOLE_CLASS	 0xFF
#define USB_GOOGLE_CONSOLE_SUBCLASS  0x50
#define USB_GOOGLE_CONSOLE_PROTOCOL  0x01
...
//Define state for operation.
#define DevStayInDetecting			(1<<0)
#define DevStayInOldBldr			(1<<1)	
#define DevStayInBldrWritter		(1<<2)
#define DevStayInBldr				(1<<3)
#define DevStayInAppWithConsule		(1<<4)
#define DevStayInAppWithBillBoard	(1<<5)
#define DevStayInDfuBldr			(1<<6)
#define DevStayInUnknownState		(1<<7)
...
//Winusb GUID
static GUID	GUID_APP = { 0x58D07210, 0x27C1, 0x11DD, { 0xBD, 0x0B, 0x08, 0x00, 0x20, 0x0C, 0x9A, 0x66 } };
...
void CDfuSeDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_CHECK_SKIP_SIG, m_SkipSigCheck);
	DDX_Check(pDX, IDC_CHECK_SIG, m_SetCheckSig);
...
int CDfuSeDemoDlg::IsDFUDevice(HANDLE hDle)
{
	UINT NbOfInterfaces = 0;
	USB_INTERFACE_DESCRIPTOR IntDesc;

	if (STDFU_GetNbOfInterfaces(&hDle, 0, &NbOfInterfaces) == STDFU_NOERROR)
	{
		for (int i = 0; i < NbOfInterfaces; i++)
		{
			if (STDFU_GetInterfaceDescriptor(&hDle, 0, i, 0, &IntDesc) == STDFU_NOERROR)
			{
				if ((IntDesc.bInterfaceClass == 0xFE) &&
					(IntDesc.bInterfaceProtocol == 0x02) &&
					(IntDesc.bInterfaceSubClass == 0x01))
				{
					//DFU device
					return 1;
				}
...
void CDfuSeDemoDlg::OnSelchangeCombodevices() 
{
	...
	if (Sel!=LB_ERR)
	{
		...
		if ((Sel >= (int)m_HidDevice_Counter) && (IsDFUDevice(hDle)))
		{
			m_CtrlDFUDevices.GetText(Sel, m_CurrDFUName);
			//to be removed
...
//=========================================================================================================================
		...
		else //else HID device
		{
			...
			Tmp.Format("%04X", m_DeviceDesc.bcdDevice);
			m_CtrlDevAppBcd.SetWindowText(Tmp);
			UpdateData(FALSE);//checkbox will be set.
#endif
		}
	}
}
...
void CDfuSeDemoDlg::OnButtonenterdfu() 
{
	...
	{
			UpdateData(TRUE);
			// Application state,send enter DFU command.
			...
			Context.Operation = OPERATION_REBOOT;
			Context.hImage=NULL;
			Context.RebootFlag = 0;

			if (m_SkipSigCheck)
				Context.RebootFlag |= REBOOT_FLAG_SET_SIGVERIFY;

			if (m_SetCheckSig)
				Context.RebootFlag |= REBOOT_FLAG_CLEAR_SIGVERIFY;

			Context.RebootFlag |= REBOOT_FLAG_SET_DFU;
	...
	if ((nEventType == DBT_DEVICEREMOVECOMPLETE))// || (nEventType == 0x0007))
	{
		...
		if (hdr->dbcd_devicetype==DBT_DEVTYP_DEVICEINTERFACE)
		{
			...
			if ( (strcmpi(p->dbcc_name, m_CurrDFUName)==0) )//&& (m_OperationCode!=0) )
			{
				...
				if ((Context.Operation != OPERATION_RETURN) && (Context.Operation != OPERATION_DETACH) && (Context.Operation != OPERATION_REBOOT))
				{
				...
				else if(Context.Operation == OPERATION_REBOOT){
