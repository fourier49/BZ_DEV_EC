...
void pd_set_dual_role_port(int port,enum pd_dual_role_states state)
{
	...
	for (i = 0; i < PD_PORT_COUNT; i++) {
		...
		if (pd[i].power_role == PD_ROLE_SOURCE &&
		    (pd[i].drp_state == PD_DRP_FORCE_SINK ||
		     (pd[i].drp_state== PD_DRP_TOGGLE_OFF
		...
		if (pd[i].power_role == PD_ROLE_SINK &&
		    pd[i].drp_state == PD_DRP_FORCE_SOURCE) {
...
void pd_task(void)
{
	...
	while (1) {
		...
		case PD_STATE_SRC_DISCONNECTED:
			...
			else if (pd[port].drp_state != PD_DRP_FORCE_SOURCE &&
		...
		case PD_STATE_SNK_DISCONNECTED:
			 ...
			 */
			if (pd[port].drp_state== PD_DRP_TOGGLE_ON &&
			    get_time().val >= next_role_swap &&
...
static int command_aux(int argc, char **argv)
{
	...
	ccprintf("Port C%d: CC1 %d mV  CC2 %d mV (polarity:CC%d)\n",
		port, pd_adc_read(port, 0), pd_adc_read(port, 1),
		pd_get_polarity(port) + 1);
	ccprintf("HPD:%d AUX_N/P:%d/%d ConnSts:%d (%d,%d)\n", gpio_get_level(GPIO_DP_HPD),
		adc_read_channel(ADC_CH_AUX_N), adc_read_channel(ADC_CH_AUX_P),
		dpe_plug_conn_status(port), dpe_conn_status, tce_conn_status);
	return EC_SUCCESS;
}
...
static int command_amode(int argc, char **argv)
{
	...
	case 3: // Config 4 lanes Mode
		...
		board_set_usb_mux(port, (cmd==3)? TYPEC_MUX_DP : TYPEC_MUX_DOCK, USB_SWITCH_CONNECT,
			pd_get_polarity(port));
		break;
	default:
